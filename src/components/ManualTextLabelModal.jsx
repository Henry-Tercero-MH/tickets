...
import logoPilarFUrl from '../imagenes/logoPilarF.png';

// Opciones de imagen del logo
const LOGO_IMAGE_OPTIONS = [
  { id: 'pilar', label: 'Logo Pilar', url: logoPilarUrl },
  { id: 'pilarF', label: 'Logo Pilar F', url: logoPilarFUrl },
];
...
const ManualTextLabelModal = ({ open, onClose }) => {
  ...
  const [selectedLogoId, setSelectedLogoId] = useState('pilar');
  const [logoBase64, setLogoBase64] = useState('');
  const [bulkMode, setBulkMode] = useState(false);
  const [bulkText, setBulkText] = useState('');

  // URL del logo seleccionado
  const currentLogoUrl = selectedLogoId === 'pilarF' ? logoPilarFUrl : logoPilarUrl;

  // Convertir logo a base64 para el iframe de impresión
  useEffect(() => {
    fetch(currentLogoUrl)
      .then(res => res.blob())
      .then(blob => {
    ...
    if (isLogoOnly) {
      return (
        <div className="flex items-center justify-center w-full h-full">
          <img src={currentLogoUrl} alt="Logo" style={{ height: '70%', width: 'auto', objectFit: 'contain' }} />
        </div>
      );
    }

    const logoImg = (
      <img src={currentLogoUrl} alt="Logo" style={{ height: '55%', width: 'auto', objectFit: 'contain', flexShrink: 0 }} />
    );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-xl shadow-2xl max-w-3xl w-full relative overflow-hidden">
        ...
        <div className="p-6">
              ...
              {LOGO_OPTIONS.map((opt) => (
                ...
                >
                  ...
                  <span className="flex items-center gap-0.5 text-[10px]">
                    ...
                    {opt.id === 'only' && (
                      <img src={currentLogoUrl} alt="" className="h-5 w-auto" />
                    )}
                    {opt.id === 'start' && (
                      <>
                        <img src={currentLogoUrl} alt="" className="h-4 w-auto" />
          ...
          {/* Selector de imagen del logo (visible cuando se usa logo) */}
          {logoPosition !== 'none' && (
            <div className="mb-4">
              <label className="block text-sm font-semibold text-gray-700 mb-2">
                Imagen del logo
              ...
              <div className="flex gap-3 flex-wrap items-center">
                {LOGO_IMAGE_OPTIONS.map((opt) => (
                  <button
                    key={opt.id}
                    onClick={() => setSelectedLogoId(opt.id)}
                    className={`flex flex-col items-center gap-1 px-3 py-2 rounded-lg border transition-all duration-200 ${
                      selectedLogoId === opt.id
                        ? 'bg-green-600 border-green-600 shadow-md ring-2 ring-green-300'
                  ...
                  >
                    ...
                    <span className={`text-xs font-medium ${
                      selectedLogoId === opt.id ? 'text-white' : 'text-gray-600'
                    }`}>{opt.label}</span>
                  </button>
                ))}
              </div>
